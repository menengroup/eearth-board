"""
EEARTH Board of Directors â FastAPI Backend
...
# âââââââââââââââââââ App setup âââââââââââââââââââ
...
# âââââââââââââââââââ USER MANAGEMENT (Admin only) âââââââââââââââââââ
...
# âââââââââââââââââââ BOARD MEMBERS âââââââââââââââââââ
...
# âââââââââââââââââââ MEETINGS âââââââââââââââââââ
...
# âââââââââââââââââââ DOCUMENT UPLOAD âââââââââââââââââââ
...
# âââââââââââââââââââ CHAT (1-on-1 and Document Review) âââââââââââââââââââ
...
# âââââââââââââââââââ BOARD MEETING â ALL MEMBERS RESPOND âââââââââââââââââââ
...
# âââââââââââââââââââ GENERATE MINUTES âââââââââââââââââââ
...
async def generate_minutes(meeting_id: int, user: dict = Depends(get_current_user)):
    ...
    directors_present = ", ".join(members_present) if members_present else "Full Board"
    minutes_prompt = f"""You are the Corporate Secretary of EEARTHâ¢ Exploration LLC. Generate formal, professional Board of Directors meeting minutes based on the following transcript.

MEETING DETAILS:
- Meeting Title: {meeting_title}
- Date: {meeting_date}
- Time: {meeting_time}
- Format: Board of Directors Meeting
- Directors Present: {directors_present}

The minutes must be comprehensive and follow this exact structure:

âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
EEARTHâ¢ EXPLORATION LLC
BOARD OF DIRECTORS â MEETING MINUTES
âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

I. CALL TO ORDER
[State that the meeting was duly called to order, date, time]

II. DIRECTORS PRESENT
[List each director present with their full name and title]

III. QUORUM
[Confirm quorum established]

IV. AGENDA AND DISCUSSION

[For EACH topic discussed in the transcript, create a numbered section:]
[Section Number]. [TOPIC HEADING IN CAPS]

Discussion Summary:
[2-4 sentence summary of the key points raised]

Director Positions:
â¢ [Director Name]: [Their key position/recommendation in 1-2 sentences]
â¢ [Director Name]: [Their key position/recommendation in 1-2 sentences]
[...for each director who spoke]

V. RESOLUTIONS
[List any formal decisions reached. Format each as:]
RESOLVED, that [specific action/decision].
Vote: [APPROVED UNANIMOUSLY / APPROVED [X] to [Y] / TABLED]

VI. ACTION ITEMS
[Specific next steps:]
â¢ [Action item] â Responsible Party: [Name/Role] â Target Date: [If mentioned]

VII. NEXT MEETING
[To be scheduled at the discretion of the Chair.]

VIII. ADJOURNMENT
[State the meeting was adjourned.]

_________________________________     _________________________________
Chris Milligan, PE                    Board Secretary
Executive Director / CEO              EEARTHâ¢ Exploration LLC
Date: {meeting_date}
...
Generate complete, formal corporate minutes following the structure above exactly."""
...
# âââââââââââââââââââ MEMORY MANAGEMENT âââââââââââââââââââ
...
async def update_memory_from_meeting(
    ...
    """Auto-generate and update each board member's memory based on this meeting."""
    meeting = db.get_meeting(meeting_id, user["id"])
    if not meeting:
        raise HTTPException(404, "Meeting not found")
    ...
    # Determine which members participated
    ...
    for member_key in target_keys:
        ...
        # Build this member's transcript
        member_transcript = []
        for msg in messages:
            if msg["role"] == "user":
        ...
        memory_prompt = f"""You are generating a memory update for {member_name}, a Board Director of EEARTH.
This memory will be loaded into {member_name}'s context in future meetings so they can reference past discussions.

EXISTING MEMORY:
{existing_memory}

NEW MEETING TRANSCRIPT (Meeting: "{meeting["title"]}", Date: {time.strftime("%B %d, %Y")}):
{chr(10).join(member_transcript[:3000])}

Generate an updated, comprehensive memory document for {member_name}. This should be written in first person as {member_name} would remember it. Include:
1. Key topics discussed and {member_name}'s positions taken
2. Important facts about EEARTH's progress or challenges raised
3. Outstanding concerns or action items {member_name} is tracking
4. Notable positions or insights from other board members (reference them by name)
5. Any commitments made or follow-ups requested

Write this as a clear, organized memory that will help {member_name} be a more effective board member in future sessions. Keep it under 800 words but be specific."""
...
# âââââââââââââââââââ FOUNDATION MEMORY (read-only, admin-viewable) âââââââââââââââââââ

@app.get("/api/foundation-memory")
async def get_foundation_memory_all(user: dict = Depends(get_current_user)):
    """Return the foundation EEARTH knowledge base for all board members (always-on context)."""
    memories = get_all_foundation_memories()
    return {
        "description": "Foundation EEARTH knowledge permanently embedded in all board member contexts.",
        "members": {
            key: {
                "name": BOARD_MEMBERS.get(key, {}).get("name", key),
...
async def get_foundation_memory_member(board_member: str, user: dict = Depends(get_current_user)):
    """Return the full foundation memory for a specific board member."""
    if board_member not in BOARD_MEMBERS:
        raise HTTPException(400, f"Unknown board member: {board_member}")
    content = get_foundation_memory(board_member)
    return {
        "board_member": board_member,
        "name": BOARD_MEMBERS[board_member]["name"],
        "content": content,
    }


# âââââââââââââââââââ STATIC FILES / SPA âââââââââââââââââââ
...
async def serve_spa(path: str):
    ...
    return HTMLResponse("<h1>EEARTH Board â frontend not built yet</h1>", status_code=503)
